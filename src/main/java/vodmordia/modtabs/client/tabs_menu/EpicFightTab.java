...
/**
 * Tab for Epic Fight's skill editor (Forge 1.20.1). Mirrors the mod's own "K" keybind
 * path — {@code ControlEngine.openSkillEditor()} just calls
 * {@code Minecraft.setScreen(new SkillEditScreen(player, patch.getSkillCapability()))}
 ...
 * which reads Epic Fight's Forge capability. If the patch or its skill capability isn't
 * ready yet (e.g. player just logged in), the click is a no-op.
 */
...
public class EpicFightTab extends IntegrationItemTab {
    private static final ResourceLocation SKILLBOOK_ID =
            new ResourceLocation("epicfight", "skillbook");
    ...
    public void openTargetScreen(Player player) {
        ...
        try {
            ...
            Method getSkills = patch.getClass().getMethod("getSkillCapability");
            ...
            // SkillEditScreen's second parameter is declared as CapabilitySkill (the parameter
            // type returned by getSkillCapability()), not whatever concrete subclass the
            // instance is — so resolve by name rather than skills.getClass().
