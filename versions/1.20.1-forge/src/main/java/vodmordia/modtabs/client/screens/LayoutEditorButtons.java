...
public final class LayoutEditorButtons {
    ...
    public static void addToScreen(Screen screen, java.util.function.Consumer<AbstractWidget> add) {
        ...
        if (configKey != null) {
            ...
            // Icon-nudge row (row 6): four small numeric inputs for per-direction px offsets.
    ...
    /**
     * Toggle button for tab visual order (LEFT_TO_RIGHT / RIGHT_TO_LEFT). Pairs with a
     * 180° rotation: RIGHT_TO_LEFT lands the inventory tab at the rightmost natural-frame
     * slot so it appears on the screen-left after rotation.
     */
    ...
    public static class MaxTabsPerPageEditBox extends EditBox {
        ...
        public void setFocused(boolean focused) {
            ...
            // On focus-lost, re-init so the bar's pagination picks up the typed value.
            // The responder writes on every keystroke; deferring re-init avoids tearing
            // down the focused widget and dropping characters.
     ...
     * integer percentage (default 100). Empty/invalid input is treated as 100 so the
     * icon doesn't disappear mid-edit.
     */
